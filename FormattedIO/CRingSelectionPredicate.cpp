...
CRingSelectionPredicate::operator()(CRingBuffer& ring)
{
  ...
  if (ring.availableData() < header.size()) {
  ...
  ring.peek(header.data(), header.size());

  uint32_t size, type;
  bool swapNeeded;
  V12::Parser::parseSizeAndType(header.begin(), header.end(),
                                size, type, swapNeeded);
  ...
  if (selectThis(type)) {
    ...
    ring.skip(size); // no need to block here.
	return true;
      }
    } else {
...
uint32_t CRingSelectionPredicate::longswap(uint32_t value)
{
    DAQ::BO::CByteSwapper swapper(true);
    return swapper.copyAs<uint32_t>(reinterpret_cast<char*>(&value));
}
