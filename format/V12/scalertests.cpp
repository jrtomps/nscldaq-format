...
void scltests::simplecons() {
  ...
  EQMSG("unix timestamp", (uint32_t)time(NULL), s.getTimestamp());
  EQMSG("scaler count", (uint32_t)32, s.getScalerCount());
  EQMSG("incremental", (uint32_t)1, s.isIncremental());
  EQMSG("interval divisor", (uint32_t)1, s.getIntervalDivisor());
